...
- Bei neuen Fehlerklassen (Build/Test/Runtime/Config): zuerst einen minimalen, schnellen Sanity-Check definieren, der
  den
...
- **Abschluss-Regel**: Das gesamte Ticket/Issue gilt erst dann als erfolgreich bearbeitet, wenn nach dem finalen Push
  alle relevanten Jobs der CI-Pipeline fehlerfrei (grün) abgeschlossen wurden. Eine manuelle Bestätigung oder das
  Signalisieren der Fertigstellung darf erst nach diesem positiven CI-Status erfolgen.
...
- Danach: Fehler neu klassifizieren, neue Evidenz erzwingen (anderer Proof-Command, anderer Gate-Step, andere
  Datei/Quelle).
