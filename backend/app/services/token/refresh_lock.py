...
class RefreshTokenLockManager:
    ...
    def acquire(self, token_digest: str) -> Generator[None, None, None]:
        ...
        LOG.debug("acquiring_refresh_lock", extra={"token_digest_prefix": token_digest[:8]})
        
        # Token-spezifischen Lock erwerben
        token_lock.acquire()
        try:
            LOG.debug("refresh_lock_acquired", extra={"token_digest_prefix": token_digest[:8]})
            yield
        finally:
            token_lock.release()
            LOG.debug("refresh_lock_released", extra={"token_digest_prefix": token_digest[:8]})
            ...
            with self._manager_lock:
                ...
                if token_digest in self._locks and not self._locks[token_digest].locked():
                    ...
                    LOG.debug("refresh_lock_cleaned_up", extra={"token_digest_prefix": token_digest[:8]})
