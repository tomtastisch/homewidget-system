#!/usr/bin/env bash
set -Eeuo pipefail

## @file run_steps.sh
## @brief Lokaler Pipeline-Runner, der auf ci_steps.sh aufsetzt.
##
## Beispiel:
##   tools/dev/orchestration/run_steps.sh all
##   tools/dev/orchestration/run_steps.sh backend
##   tools/dev/orchestration/run_steps.sh mobile
##   tools/dev/orchestration/run_steps.sh tests

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
CI_STEPS="${SCRIPT_DIR}/../pipeline/ci_steps.sh"

if [[ ! -f "${CI_STEPS}" ]]; then
    echo "[run-steps][ERROR] ci_steps.sh wurde nicht gefunden unter ${CI_STEPS}" >&2
    exit 1
fi

usage() {
    cat <<USAGE
Verwendung: tools/dev/orchestration/run_steps.sh [all|backend|mobile|tests|mobile_jest_tests]

  all        Vollständige Pipeline (Backend + Mobile)
  backend    Nur Backend-Pipeline
  mobile     Nur Mobile-Pipeline
  tests      Nur Test-Pipeline
  mobile_jest_tests  Nur die mobilen Jest-Tests (entspricht ci_steps.sh mobile_jest_tests)
USAGE
}

main() {
    local target="${1:-all}"

    case "${target}" in
        all)
            bash "${CI_STEPS}" pipeline_all
            ;;
        backend)
            bash "${CI_STEPS}" pipeline_backend
            ;;
        mobile)
            bash "${CI_STEPS}" pipeline_mobile
            ;;
        tests)
            bash "${CI_STEPS}" pipeline_tests
            ;;
        mobile_jest_tests)
            bash "${CI_STEPS}" mobile_jest_tests
            ;;
        *)
            echo "[run-steps][ERROR] Unbekanntes Ziel: ${target}" >&2
            usage
            exit 1
            ;;
    esac
}

main "$@"